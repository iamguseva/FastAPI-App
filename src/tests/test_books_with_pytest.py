...
async def test_create_book(db_session, async_client):
    # Создаем продавца
    seller = essences.Seller(first_name="James", last_name="Bond", email="JamesBond@mail.com", password="password")
...
async def test_get_books(db_session, async_client):
    ...
    book = essences.Book(author="Pushkin", title="Eugeny Onegin", year=2001, count_pages=104, seller_id=1)
    book_2 = essences.Book(author="Lermontov", title="Mziri", year=1997, count_pages=104, seller_id=1)
...
async def test_delete_book(db_session, async_client):
    ...
    all_books = await db_session.execute(select(essences.Book))
...
async def test_update_book(db_session, async_client):
    ...
    res = await db_session.get(essences.Book, book.id)
